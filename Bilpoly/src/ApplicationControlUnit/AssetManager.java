package ApplicationControlUnit;

import GameLogic.*;

import GameLogic.Player;
import UserInterface.*;
import UserInterface.LandPopupController;
...
public class AssetManager extends Application {
    ...
    public static boolean boardMode;
    ...
    public void start(Stage primaryStage) throws Exception{
        ...
        scene.getStylesheets().add(getClass().getResource("../UserInterface/background.css").toString());
        ...
        //music
         URL resource = getClass().getResource("../UserInterface/music/pacific_dawn.mp3");
    ...
    private static Landable[] getLandableList(boolean boardMode) {
        ...
        //if (boardmode == false) {
            ...
            try {
                File file = new File("src/UserInterface/Landables/lands.txt");
            ...
            try {
                File file = new File("src/UserInterface/Landables/cafes.txt");
                System.out.println("Attempting to read from file in: "+ file.getCanonicalPath());
                Scanner scan = new Scanner(file);
                int index = 0;
                while (scan.hasNextLine()) {
            ...
            functionalPlaceList[1] = new FunctionalPlace(new FeeStrategy(550, "tuition")); //TUITION FEE
            functionalPlaceList[2] = new FunctionalPlace(new FeeStrategy(1100, "dorm")); //DORM FEE
            functionalPlaceList[3] = new FunctionalPlace(new FreeParkingStrategy()); //FREE PARKING
            functionalPlaceList[4] = new FunctionalPlace(new FeeStrategy(2200, "book")); //BOOK FEE
            functionalPlaceList[5] = new FunctionalPlace(new GoToAtalarStrategy()); //GO TO ATALAR
            functionalPlaceList[6] = new FunctionalPlace(new FeeStrategy(4400, "food")); //FOOD FEE
    ...
    private static CardDeck getCardDeck(boolean boardMode){
        ...
        try {
            File file = new File("src/UserInterface/Cards/cards.txt");
            System.out.println("Attempting to read from file in: "+ file.getCanonicalPath());
            Scanner scan = new Scanner(file);
            int num = 0;
            while (scan.hasNextLine()) {
